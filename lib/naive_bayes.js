 ...
 * Use this with classifier.exportToJson().
 *
 * @param  {String} jsonStr   state representation obtained by classifier.exportToJson()
 * @return {NaiveBayes}       Classifier
 */
module.exports.importFromJson = function (jsonStr) {
  var parsed;
  try {
    parsed = JSON.parse(jsonStr)
  } catch (e) {
    throw new Error('Naivebayes.importFromJson expects a valid JSON string.')
  }

  return this.importFromObject(parsed)
}

module.exports.importFromObject = function (object) {
  var parsed = object;
  ...
  STATE_KEYS.forEach(function (k) {
    if (typeof parsed[k] === 'undefined' || parsed[k] === null) {
      throw new Error('Naivebayes.importFromObject: Object is missing an expected property: `' + k + '`.')
    }
 ...
 * 
 * @param  {String} text
 * @return {Promise<Object[]>} categories {name, score}
 */
Naivebayes.prototype.categorize = async function (text, numOfAlternatives = 1, scoreThresholdForAlternatives = 0) {
  var self = this
    , chosenCategory = new Array(numOfAlternatives).fill({ name: null, score: -Infinity })
  ...
  Object
    ...
    .forEach(function (category) {
      ...
      for (let i = 0; i < chosenCategory.length; i++) {
        if (logProbability > chosenCategory[i].score) {

          chosenCategory = insert(chosenCategory, i, { name: category, score: logProbability })
          chosenCategory.pop()
          break;
        }
      }
 ...
 */
Naivebayes.prototype.exportToJson = function () {
  var jsonStr = JSON.stringify(this.exportToObject())

  return jsonStr
}
/**
 * Dump the classifier's state as a JS object.
 * @return {Object} object data of the classifier.
 */
Naivebayes.prototype.exportToObject = function () {
  var state = {}
  var self = this
  STATE_KEYS.forEach(function (k) {
    state[k] = self[k]
