 ...
 */
Naivebayes.prototype.categorize = async function (text, numOfAlternatives = 1) {
  var self = this
    , maxProbability = new Array(numOfAlternatives).fill(-Infinity)
    , chosenCategory = new Array(numOfAlternatives).fill(null)
  ...
  Object
    ...
    .forEach(function (category) {
      ...
      for (let i = 0; i < maxProbability.length; i++) {
        if (logProbability > maxProbability[i]) {

          maxProbability = insert(maxProbability, i, logProbability)
          chosenCategory = insert(chosenCategory, i, category)
